...
import { allCourses, javaLessons as javaLessonsList } from "@/lib/courses";
import { javaLessons } from "@/lib/lessons/javaLessons";
...
export default function JavaLessonPage({ params }: PageProps) {
    ...
    const lesson = javaLessons[lessonId];
    const lessonInfo = javaLessonsList.find(l => l.id === lessonId);

    useEffect(() => { if (!lesson) router.push("/tutorials/java"); }, [lesson, router]);
