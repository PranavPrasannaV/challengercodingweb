...
import { allCourses, pythonLessons as pythonLessonsList } from "@/lib/courses";
import { pythonLessons } from "@/lib/lessons/pythonLessons";

interface PageProps { params: Promise<{ id: string }>; }

export default function PythonLessonPage({ params }: PageProps) {
    const resolvedParams = use(params);
    const lessonId = parseInt(resolvedParams.id);
    const router = useRouter();
    const [currentExercise, setCurrentExercise] = useState(0);

    const lesson = pythonLessons[lessonId];
    const lessonInfo = pythonLessonsList.find(l => l.id === lessonId);

    useEffect(() => { if (!lesson) router.push("/tutorials/python"); }, [lesson, router]);
